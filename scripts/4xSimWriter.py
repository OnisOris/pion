...
def parse_args():
    ...
    parser.add_argument(
        "--file", type=str, default="swarm_trajectory.npz",
        help="Path to .npz file with arrays 'time' and 'states'"
    )
    parser.add_argument(
        "--backend", choices=["web", "qt"], default="qt",
        help="Matplotlib backend: 'web' (WebAgg) or 'qt' (QtAgg)"
    )
    parser.add_argument(
        "--width", type=float, default=16,
        help="Figure width in inches"
    )
    parser.add_argument(
        "--height", type=float, default=9,
        help="Figure height in inches"
    )
    parser.add_argument(
        "--bound", type=float, default=5,
        help="Axis limit for X, Y, Z"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-fps", type=float, default=30,
        help="Maximum frames per second for rendering"
    )
    parser.add_argument(
        "--marker-size", type=float, default=8,
        help="Size of markers in plots"
    )
    parser.add_argument(
        "--show-legend", action="store_true",
        help="Show drone IDs legend"
    )
...
def main():
    ...
    adjusted_dt = original_dt * args.decimate
    ...
    scat_top = ax_top.scatter(initial_pos[:, 0], initial_pos[:, 1],
                              s=marker_size ** 2, c='blue')
    scat_front = ax_front.scatter(initial_pos[:, 1], initial_pos[:, 2],
                                  s=marker_size ** 2, c='green')
    scat_side = ax_side.scatter(initial_pos[:, 0], initial_pos[:, 2],
                                s=marker_size ** 2, c='red')
    ...
    # Настройка осей
    for ax, view in zip([ax_top, ax_front, ax_side, ax_3d],
                        ['top', 'front', 'side', '3d']):
        configure_axis(ax, view, args.bound)

    # Информационная панель
    info_text = fig.text(0.5, 0.93, "", ha='center', fontsize=9,
                         bbox=dict(facecolor='white', alpha=0.8))

    # Легенда
    if args.show_legend and N <= 50:
        handles = [Line2D([], [], marker='o', color='w', markerfacecolor='b',
                          markersize=6, label=str(i)) for i in range(N)]
        fig.legend(handles=handles, title="Drone IDs", loc='center right',
                   ncol=1, fontsize='xx-small', bbox_to_anchor=(0.98, 0.5))
