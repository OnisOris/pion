...
class ControlConsole:
    ...
    def __init__(self,
        ...
        self.visualizer = SwarmVisualizer(self.sim, bound=max_bound, update_ms=int(dt*1000))
    ...
    def start(self):
        if self.sim_thread and self.sim_thread.is_alive():
            print("Симуляция уже запущена."); return
        self.sim.simulation_turn_on = True
        self.sim_thread = threading.Thread(target=self.sim.start_simulation_while, daemon=True)
    ...
    def process(self, line: str):
        ...
        if not ids:
            print(f"Неизвестная цель '{target}'"); return

        if cmd == 'set_speed' and len(parts) == 6:
            vx, vy, vz, yr = map(float, parts[2:6])
            for i in ids: self.sim.t_speed[i] = [vx, vy, vz, yr]
            ...
            for i in ids: self.sim.t_position[i, 2] = 1.0
            print(f"takeoff для {ids}")
        elif cmd == 'land':
            for i in ids: self.sim.t_position[i, 2] = 0.0
            print(f"land для {ids}")
        elif cmd == 'arm':
            for i in ids: self.armed[i] = True
            print(f"arm для {ids}")
        elif cmd == 'disarm':
            for i in ids: self.armed[i] = False
    ...
    def console_loop(self):
        print("Консоль управления запущена. Введите 'start', 'stop', 'exit', или таргет команду.")
        while True:
            try:
                line = input("Command> ")
