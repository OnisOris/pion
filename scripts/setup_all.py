...
def install_ubuntu_dependencies():
    ...
    print(
        "Ubuntu: Обновляем списки пакетов и устанавливаем build-essential и python3-dev..."
    )
    try:
...
def install_arch_dependencies():
    ...
    try:
        subprocess.run(
            ["pacman", "-Syu", "--noconfirm", "base-devel", "git"], check=True
        )
...
def install_windows_dependencies():
    ...
    if cl_path:
        ...
        if not choco_path:
            print(
                "Chocolatey не установлен. Пожалуйста, установите Visual Studio Build Tools вручную:"
            )
            print("https://visualstudio.microsoft.com/visual-cpp-build-tools/")
            sys.exit(1)
        else:
            print(
                "Chocolatey найден. Пытаемся установить Visual Studio Build Tools..."
            )
            try:
                ...
                if result.returncode in (0, 3010):
                    if result.returncode == 3010:
                        print(
                            "Visual Studio Build Tools установлены, но требуется перезагрузка."
                        )
                    else:
                        print("Visual Studio Build Tools установлены.")
                else:
                    print(
                        "Ошибка установки Visual Studio Build Tools через Chocolatey: Return code",
                        result.returncode,
                    )
                    sys.exit(1)
            except subprocess.CalledProcessError as e:
                print(
                    "Ошибка установки Visual Studio Build Tools через Chocolatey:",
                    e,
                )
                sys.exit(1)
...
def install_macos_dependencies():
    ...
    print(
        "macOS: Проверяем наличие Homebrew и устанавливаем необходимые зависимости..."
    )
    brew_path = shutil.which("brew")
    if not brew_path:
        print(
            "Homebrew не найден. Пожалуйста, установите Homebrew с https://brew.sh/ и запустите скрипт снова."
        )
        sys.exit(1)
    try:
        ...
        print(
            "Устанавливаем необходимые пакеты через Homebrew: python3 и git..."
        )
...
def install_python_package():
    ...
    if not os.path.exists(venv_dir):
        ...
        try:
            subprocess.run(
                [python_executable, "-m", "venv", venv_dir], check=True
            )
        except subprocess.CalledProcessError as e:
    ...
    try:
        subprocess.run(
            [
                pip_executable,
                "install",
                "git+https://github.com/OnisOris/pion",
            ],
            check=True,
        )
...
def main():
    ...
    if current_os == "Linux":
        ...
        print(
            ...
            current_os,
            "не поддерживается данным скриптом.",
        )
        sys.exit(1)
