...
class SwarmVisualizer4D:
    ...
    def process_payload(self, payload, addr):
        with self.lock:
            ...
            position = np.array([payload.data[1], payload.data[2], payload.data[3]])
            velocity = np.array([payload.data[4], payload.data[5], payload.data[6]])
            attitude = np.array(payload.data[7:13]) if len(payload.data) >= 13 else np.zeros(6)
            t_speed = np.array(payload.data[13:17]) if len(payload.data) >= 17 else np.zeros(4)
    ...
    def draw_vectors_2d(self, ax, pos, velocity, factor=0.5, color="red"):
        ...
        ax.quiver(pos[0], pos[1], arrow_dx, arrow_dy, color=color, angles="xy", scale_units="xy", scale=1, width=0.005, headwidth=5, headlength=7, alpha=0.8)

    def draw_vectors_3d(self, ax, pos, velocity, factor=0.5, color="red"):
        ax.quiver(pos[0], pos[1], pos[2], velocity[0] * factor, velocity[1] * factor, velocity[2] * factor, color=color, length=1.0, normalize=False)
    ...
    def update_plot(self, frame):
        with self.lock:
            ...
            for drone_key in list(self.drones.keys()):
                ...
                self.ax_top.scatter(pos[0], pos[1], c=[color], s=80, marker="o", edgecolors="k")
                if len(data["trail"]) > 1:
                    trail = np.array(data["trail"])
                    self.ax_top.plot(trail[:, 0], trail[:, 1], c=color, linestyle=":", alpha=0.6, linewidth=1)
                self.draw_vectors_2d(self.ax_top, pos, velocity, factor=0.2, color="red")
                self.ax_front.scatter(pos[1], pos[2], c=[color], s=80, marker="o", edgecolors="k", label=data["short_id"])
                if len(data["trail"]) > 1:
                    trail = np.array(data["trail"])
                    self.ax_front.plot(trail[:, 1], trail[:, 2], c=color, linestyle=":", alpha=0.6, linewidth=1)
                vel_front = np.array([velocity[1], velocity[2]])
                self.ax_front.quiver(pos[1], pos[2], vel_front[0] * 0.2, vel_front[1] * 0.2, color="red", angles="xy", scale_units="xy", scale=1, width=0.005, headwidth=5, headlength=7, alpha=0.8)
                self.ax_side.scatter(pos[0], pos[2], c=[color], s=80, marker="o", edgecolors="k")
                if len(data["trail"]) > 1:
                    trail = np.array(data["trail"])
                    self.ax_side.plot(trail[:, 0], trail[:, 2], c=color, linestyle=":", alpha=0.6, linewidth=1)
                vel_side = np.array([velocity[0], velocity[2]])
                self.ax_side.quiver(pos[0], pos[2], vel_side[0] * 0.2, vel_side[1] * 0.2, color="red", angles="xy", scale_units="xy", scale=1, width=0.005, headwidth=5, headlength=7, alpha=0.8)
                self.ax_3d.scatter(pos[0], pos[1], pos[2], c=[color], s=80, marker="o", edgecolors="k")
                if len(data["trail"]) > 1:
                    trail = np.array(data["trail"])
                    self.ax_3d.plot(trail[:, 0], trail[:, 1], trail[:, 2], c=color, linestyle=":", alpha=0.6, linewidth=1)
                self.draw_vectors_3d(self.ax_3d, pos, velocity, factor=0.2, color="red")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Запуск 4D визуализации дронов")
    parser.add_argument("--web", action="store_true", help="Запуск в web режиме")
    parser.add_argument("--width", type=float, default=12, help="Ширина графика (размер фигуры)")
    parser.add_argument("--height", type=float, default=10, help="Высота графика (размер фигуры)")
    parser.add_argument("--bound", type=float, default=5, help="Предел по осям (в метрах)")
    args = parser.parse_args()
    if args.web:
        matplotlib.use("WebAgg")
    visualizer = SwarmVisualizer4D(port=37020, fig_width=args.width, fig_height=args.height, bound=args.bound)
    try:
        visualizer.run()
